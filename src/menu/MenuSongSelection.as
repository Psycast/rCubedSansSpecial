	...
	public class MenuSongSelection extends MenuPanel {
		...
		override public function stageRemove():void {
			...
			if (stage) {
				...
				if(scrollbar) scrollbar.removeEventListener(Event.CHANGE, scrollBarMoved, false);
			}
		}
		...
		public function buildGenreList():void {
			...
			if (GENRE_MODE == GENRE_DIFFICULTIES) {
				...
				for (gindex = -1; gindex < max_diff_genres; gindex++) {
					if (gindex == -1) str = _lang.string("genre_" + gindex);
					else str = _lang.string("difficulty_title_" + gindex);
				...
				for (gindex = -1; gindex < max_flags_genres; gindex++) {
					...
					if (gindex >= 0) str = GlobalVariables.SONG_ICON_TEXT[gindex];
					if (gindex == 1) str = "PLAYED";
					if (options.activeGenre == gindex) {
		...
		public function buildPlayList():void {
			...
			for (var sX:int = 0; sX < songList.length; sX++) {
				...
				// Locked Song
					...
					sI = new SongItemLocked(song, getSongLockText(song["access"], song), (song["access"] == GlobalVariables.SONG_ACCESS_PURCHASED ? "http://www.flashflashrevolution.com/tools/ffrshop.php" : ""));
					sI.y = yOffset;
		...
		public function buildInfoTab():void {
			...
			if (options.infoTab == TAB_SEARCH) { // Song Search
			...
			else if (options.infoTab == TAB_QUEUE) { // Playlist Queue
			...
			else if (options.infoTab == TAB_HIGHSCORES) { // Song Ranks
		...
		private function clickHandler(e:Event):void {
			if (e.target == genre_mode_prev || e.target == genre_mode_next) {
				...
				if (GENRE_MODE < 0) GENRE_MODE = GENRE_MODES;
				if (GENRE_MODE > GENRE_MODES) GENRE_MODE = 0;
		...
		public function multiplayerSelect(songName:String, song:Object):void {
			...
			options.isFilter = true;
			if (song)
				options.filter = function(fsong:Object):Boolean { return fsong.level == song.level; };
			else
				options.filter = function(fsong:Object):Boolean { return fsong.name == songName; };
			options.infoTab = TAB_PLAYLIST;
			buildGenreList();
			buildPlayList();
			buildInfoTab();
