package popups {
	...
	public class PopupOptions extends MenuPanel {
		...
		private function renderMenu():void {
			...
			editorOptionsMP.editor_multiplayer = {
				playerCount: 2,
			...
			editorOptionsMPSpec.editor_multiplayer = {
				playerCount: 2,
		...
		private function changeHandler(e:Event):void {
			if (e.target == optionGameSpeed) {
				...
				if (gid == 0) _gvars.activeUser.gameColours[2] = ColorUtil.darkenColor(newColorG, 0.27);
				if (gid == 1) _gvars.activeUser.gameColours[3] = ColorUtil.brightenColor(newColorG, 0.08);
				...
				switch (e.target.key_name) {
					case "optionJudgeColor": 		sourceArray = optionJudgeColors; break;
					case "gameComboColorDisplay": 	sourceArray = optionComboColors; break;
					case "gameGameColorDisplay": 	sourceArray = optionGameColors; break;
				}
		...
		private function clickHandler(e:MouseEvent):void {
			...
			// Game Background Color Reset
				...
				if (gid == 0) _gvars.activeUser.gameColours[2] = ColorUtil.darkenColor(DEFAULT_OPTIONS.gameColours[gid], 0.27);
				if (gid == 1) _gvars.activeUser.gameColours[3] = ColorUtil.brightenColor(DEFAULT_OPTIONS.gameColours[gid], 0.08);
			}
			//- Editor
		...
		private function engineRefresh():void {
			...
			engineCombo.addItem({label: "FlashFlashRevolution", data: null});
			engineDefaultCombo.addItem({label: "FlashFlashRevolution", data: null});
