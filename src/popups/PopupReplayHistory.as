package popups {
	...
	public class PopupReplayHistory extends MenuPanel {
		...
		CONFIG::air private var FILE_TRACK:FileTracker;
		...
		override public function stageAdd():void {
			...
			for each(var r:Replay in _gvars.replayHistory) {
				if (r.isValid()) {
					var engineID:String = (r.settings.arc_engine ? r.settings.arc_engine.engineID : "ffr");
		...
		CONFIG::air private function e_loadExternalQueue(e:Event):void {
		...
		private function drawEngineList():void {
			...
			if(DRAW_EXTERNAL) {
				...
				// Move FFR to the top.
				ffrIndex = -1;
				if((ffrIndex = EXTERNAL_REPLAYS_LIST.indexOf("ffr")) >= 1) {
					EXTERNAL_REPLAYS_LIST.splice(ffrIndex, 1);
					EXTERNAL_REPLAYS_LIST.unshift("ffr");
				}
			}
			else {
				...
				// Move FFR to the top.
				ffrIndex = -1;
				if((ffrIndex = INTERNAL_REPLAYS_LIST.indexOf("ffr")) >= 2) {
					INTERNAL_REPLAYS_LIST.splice(ffrIndex, 1);
					INTERNAL_REPLAYS_LIST.insertAt(1, "ffr");
				}
			}
...
internal class ReplayBox extends Sprite {
	...
	public function ReplayBox(p:PopupReplayHistory, r:Replay, s:Object):void {
		...
		if(replay.isEdited) box.color = 0xff0000;
		...
		if(!r.fileReplay) box.addChild(deleteBtn);
