		...
		private function playlistLoadComplete(e:Event):void {
			...
			for (var a:Object in data) {
				...
				if (songData["stepauthor"].indexOf(" & ") !== false) {
					...
					songData.stepauthorwithurl = "<a href=\"http://www.flashflashrevolution.com/profile/" + Crypt.urlencode(stepAuthors[0]) + "\">" + stepAuthors[0] + "</a>";
					for (var i:int = 1; i < stepAuthors.length; i++) {
						songData.stepauthorwithurl += " & <a href=\"http://www.flashflashrevolution.com/profile/" + Crypt.urlencode(stepAuthors[i]) + "\">" + stepAuthors[i] + "</a>";
					}
				} else {
					songData.stepauthorwithurl = "<a href=\"http://www.flashflashrevolution.com/profile/" + Crypt.urlencode(songData["stepauthor"]) + "\">" + songData["stepauthor"] + "</a>";
				}
