package classes.chart {
	...
	public class Song extends EventDispatcher {
		...
		public function unload():void {
			...
			isMusic2Loaded = true;
			loadFail = true;
			if (musicLoader && isMusicLoaderLoading) { musicLoader.close(); isMusicLoaderLoading = false; }
			if (chartLoader && isChartLoaderLoading) { chartLoader.close(); isChartLoaderLoading = false; }
			music = null;
			chart = null;
		}
		...
		public function getMusicContentLoader(isLoader:Boolean = false):Object {
			if (isLoader) return musicLoader.contentLoaderInfo;
		...
		private function musicCompleteHandler(e:Event):void {
			...
			if (type == NoteChart.FFR_MP3) {
				...
				CONFIG::air {
					...
					if (_gvars.air_useLocalFileCache) { // && !this.entry.engine) {
						// Alt Engine has Data
				...
				// Store SWF
				CONFIG:: air {
					if (_gvars.air_useLocalFileCache && storeChartData) { // && !this.entry.engine) {
						try {
