package classes.replay
{
	...
	/**
	 * A Class needed to pack and unpack a binary based FFR replay.
	 *
	...
	{
		/** 00000001 */ public static const BIT_P1_LEFT:int  = (1 << 0);
		/** 00000010 */ public static const BIT_P1_DOWN:int  = (1 << 1);
		/** 00000100 */ public static const BIT_P1_UP:int 	 = (1 << 2);
		/** 00001000 */ public static const BIT_P1_RIGHT:int = (1 << 3);
		/** 00000001 */ public static const BIT_P1_KEY_5:int = (1 << 0);
		/** 00000010 */ public static const BIT_P1_KEY_6:int = (1 << 1);
		
		/** 00000100 */ public static const BIT_P2_LEFT:int  = (1 << 2);
		/** 00001000 */ public static const BIT_P2_DOWN:int  = (1 << 3);
		/** 00010000 */ public static const BIT_P2_UP:int 	 = (1 << 4);
		/** 00100000 */ public static const BIT_P2_RIGHT:int = (1 << 5);
		/** 01000000 */ public static const BIT_P2_KEY_5:int = (1 << 6);
		/** 10000000 */ public static const BIT_P2_KEY_6:int = (1 << 7);
		
		/** 00010000 */ public static const BIT_FRAME_EXTEND:int = (1 << 4);
		/** 00100000 */ public static const BIT_PACK_BYTE:int 	 = (1 << 5);
		/** 01000000 */ public static const BIT_PACK_SHORT:int 	 = (1 << 6);
		/** 10000000 */ public static const BIT_PACK_INT:int 	 = (1 << 7);
		...
		public static function pack(binNotes:Array, binBoos:Array):ByteArray
		{
			...
			for (nx = 0; nx < binBoos.length; nx++)
			{
				...
				if (CUR_TIME < 0) CUR_TIME = 0; // Should Never Happen!
		...
		{
			if (dir == 'L') return BIT_P1_LEFT;
			if (dir == 'D') return BIT_P1_DOWN;
			if (dir == 'U') return BIT_P1_UP;
			if (dir == 'R') return BIT_P1_RIGHT;
			return 0;
		}
		...
		static private function verifyReplayWrite(binReplay:ByteArray, activeUser:User, options:GameOptions, judgements:String, binReplayNotes:Array, binReplayBoos:Array, settingsEncode:String, timestamp:Number):Boolean 
		{
			...
			if (test == null) {trace("test is null, complete fail"); return false; }
			if (MAGIC != test.MAGIC) {trace("MAGIC failed comparison", MAGIC, test.MAGIC); return false; }
			if (MAJOR_VER != test.MAJOR_VER) {trace("MAJOR_VER failed comparison", MAJOR_VER, test.MAJOR_VER); return false; }
			if (MINOR_VER != test.MINOR_VER) {trace("MINOR_VER failed comparison", MINOR_VER, test.MINOR_VER); return false; }
			if (activeUser.id != test.user_id) {trace("user_id failed comparison", activeUser.id, test.user_id); return false; }
			if (options.song.id != test.song_id) {trace("song_id failed comparison", options.song.id, test.song_id); return false; }
			if (options.songRate != test.song_rate) {trace("song_rate failed comparison", options.songRate, test.song_rate); return false; }
			if (timestamp != test.timestamp) {trace("timestamp failed comparison", timestamp, test.timestamp); return false; }
			if (judgements != test.raw_judgements) {trace("raw_judgements failed comparison", judgements, test.raw_judgements); return false; }
			if (settingsEncode != test.raw_settings) {trace("raw_settings failed comparison", settingsEncode, test.raw_settings); return false; }
			
			// Compare Notes
			if (test.rep_notes == null) {trace("rep_notes is NULL"); return false; }
			if (binReplayNotes.length != test.rep_notes.length) {trace("rep_notes length doesn't match", binReplayNotes.length, test.rep_notes.length); return false; }
			...
			// Compare Boos
			if (test.rep_boos == null) {trace("rep_boos is NULL"); return false }
			if (binReplayBoos.length != test.rep_boos.length) {trace("rep_boos length doesn't match", binReplayBoos.length, test.rep_boos.length); return false; }
			compareFailure = 0 ;
