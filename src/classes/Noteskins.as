package classes
{
	...
	public class Noteskins extends EventDispatcher
	{
		...
		public function getNote(index:int, noteColor:String, direction:String):MovieClip
		{
							...
							for each (var dir:String in[direction, "D"])
							{
								if (_data[skin]["notes"][color][dir] != null)
								{
									if (_data[skin]["notes"][color][dir] is BitmapData) {
										...
										n.graphics.beginBitmapFill(_data[skin]["notes"][color][dir]);
					...
					for each (var dir:String in[get_dir, "D"])
					{
						if (_data[skin]["receptor"] != null && _data[skin]["receptor"][dir] != null)
						{
							if (_data[skin]["receptor"][dir] is BitmapData) {
								...
								return new _data[skin]["receptor"][dir];
						}
					}
				}
			}
		...
		private function noteskinSWFLoadComplete(e:Event = null):void
		{
			...
			//- Create Objects
			try { _data[noteID]["notes"]["blue"] = {"D": e.target.applicationDomain.getDefinition("assets.noteskin::note_blue") as Class}; } catch (e:Error) { trace("3:NS:" + noteID + " - No Blue Note"); }
			try { _data[noteID]["notes"]["red"] = {"D": e.target.applicationDomain.getDefinition("assets.noteskin::note_red") as Class}; } catch (e:Error) { trace("3:NS:" + noteID + " - No Red Note"); }
			try { _data[noteID]["notes"]["green"] = {"D": e.target.applicationDomain.getDefinition("assets.noteskin::note_green") as Class}; } catch (e:Error) { trace("3:NS:" + noteID + " - No Green Note"); }
			try { _data[noteID]["notes"]["yellow"] = {"D": e.target.applicationDomain.getDefinition("assets.noteskin::note_yellow") as Class}; } catch (e:Error) { trace("3:NS:" + noteID + " - No Yellow Note"); }
			try { _data[noteID]["notes"]["pink"] = {"D": e.target.applicationDomain.getDefinition("assets.noteskin::note_pink") as Class}; } catch (e:Error) { trace("3:NS:" + noteID + " - No Pink Note"); }
			try { _data[noteID]["notes"]["purple"] = {"D": e.target.applicationDomain.getDefinition("assets.noteskin::note_purple") as Class}; } catch (e:Error) { trace("3:NS:" + noteID + " - No Purple Note"); }
			try { _data[noteID]["notes"]["cyan"] = {"D": e.target.applicationDomain.getDefinition("assets.noteskin::note_cyan") as Class}; } catch (e:Error) { trace("3:NS:" + noteID + " - No Cyan Note"); }
			try { _data[noteID]["notes"]["orange"] = {"D": e.target.applicationDomain.getDefinition("assets.noteskin::note_orange") as Class}; } catch (e:Error) { trace("3:NS:" + noteID + " - No Orange Note"); }
			try { _data[noteID]["notes"]["white"] = {"D": e.target.applicationDomain.getDefinition("assets.noteskin::note_white") as Class}; } catch (e:Error) { trace("3:NS:" + noteID + " - No White Note"); }
			try { _data[noteID]["receptor"] = {"D": e.target.applicationDomain.getDefinition("assets.noteskin::receptor") as Class}; } catch (e:Error) { trace("3:NS:" + noteID + " - No Receptors"); }
		...
		private function verifyNoteSkin(noteID:String):Boolean
		{
			...
			if (_data[noteID] == null ||
				_data[noteID]["receptor"] == null || 
				(_data[noteID]["receptor"] != null && !_data[noteID]["receptor"]["D"] is Class) || 
				_data[noteID]["notes"] == null ||
				_data[noteID]["notes"]["blue"] == null ||
				(_data[noteID]["notes"]["blue"] != null && !_data[noteID]["notes"]["blue"]["D"] is Class))
			{
			...
			{
				//- Check Missing Notes
				if (_data[noteID]["notes"]["red"] == null) _data[noteID]["notes"]["red"] = _data[noteID]["notes"]["blue"];
				if (_data[noteID]["notes"]["green"] == null) _data[noteID]["notes"]["green"] = _data[noteID]["notes"]["blue"];
				if (_data[noteID]["notes"]["yellow"] == null) _data[noteID]["notes"]["yellow"] = _data[noteID]["notes"]["blue"];
				if (_data[noteID]["notes"]["pink"] == null) _data[noteID]["notes"]["pink"] = _data[noteID]["notes"]["blue"];
				if (_data[noteID]["notes"]["purple"] == null) _data[noteID]["notes"]["purple"] = _data[noteID]["notes"]["blue"];
				if (_data[noteID]["notes"]["cyan"] == null) _data[noteID]["notes"]["cyan"] = _data[noteID]["notes"]["blue"];
				if (_data[noteID]["notes"]["orange"] == null) _data[noteID]["notes"]["orange"] = _data[noteID]["notes"]["blue"];
				if (_data[noteID]["notes"]["white"] == null) _data[noteID]["notes"]["white"] = _data[noteID]["notes"]["blue"];
			}
