...
package {
	...
	import com.flashfla.utils.Imgur;
	...
	public class GlobalVariables extends EventDispatcher {
		...
		public static var imgur:Imgur = new Imgur();
			...
			public function initSQLConnection():void 
			{
				...
				function sql_openHandler(event:SQLEvent):void
				{
					...
					var params:Object = {
						...
						"offset_judge": 1,
						"set_mirror_invert": 0,
						"set_custom_offsets": 1,
						"notes": "testing2"
					};
					SQLQueries.saveSongDetails(sql_conn, params, (function(result:String):void {
						trace("callback called", result);
						
						SQLQueries.getSongDetails(sql_conn, {"engine": "ffr", "song_id": 1}, (function(result2:SQLResult):void {
							if(result2 != null)
								trace(ObjectUtil.print_r(result2.data));
						}));
					}));
					*/
				}
		...
		//- ScreenShot Handling
		/**
		 * Takes a screenshot of the stage and uploads to Imgur.
		 */
			...
			if (params.o == false) {
				try {
					...
					if (SystemUtil.isFlashNewerThan(10, 0)) {
						...
						//logDebugError("file_reference_error", e);
					}
					else {
				...
				// Make Imgur Uploader
				if (params.s == 2) {
					if (imgur.remaining >= 10 && imgur.remaining != -1) {
						gameMain.addPopup(new PopupScreenShot(gameMain, {error: "Imgur_Uploading"}), true);
						imgur.upload(b, "R^3 v" + Constant.VERSION, DateUtil.toRFC822(new Date()));
						imgur.addEventListener(Imgur.LOAD_COMPLETE, onScreenShotComplete);
						imgur.addEventListener(Imgur.LOAD_ERROR, onScreenShotError);
					} else {
						gameMain.addPopup(new PopupScreenShot(gameMain, {error: "Imgur_LimitReached", refresh: imgur.refresh}), true);
					}
				}
				// Make FFR Uploader
				if (params.s == 3) {
					gameMain.addPopup(new PopupScreenShot(gameMain, { error: "FFR_Uploading" } ), true);
					var ml:MultipartURLLoader = new MultipartURLLoader();
					ml.dataFormat = URLLoaderDataFormat.TEXT;
					ml.addEventListener(Event.COMPLETE, onScreenShotFFRComplete);
					ml.addVariable('photo_title', "R^3 v" + Constant.VERSION + " - " + DateUtil.toRFC822(new Date()));
					ml.addFile(PNGEncoder.encode(b), 'img.png', 'GameScreenshot');
					//ml.load(Constant.SCREENSHOT_URL);
				}
			}
		}
		...
		private function onScreenShotFFRComplete(e:Event):void {
			var loader:URLLoader = MultipartURLLoader(e.target).loader;
			var result:Object = JSON.parse(loader.data);
			gameMain.addPopup(new PopupScreenShot(gameMain, result), true);
		}
		
		private function onScreenShotComplete(e:Event):void {
			var result:Object = JSON.parse(e.target.RESPONSE);
			gameMain.addPopup(new PopupScreenShot(gameMain, result), true);
			imgur.removeEventListener(Imgur.LOAD_COMPLETE, onScreenShotComplete);
			imgur.removeEventListener(Imgur.LOAD_ERROR, onScreenShotError);
		}
		
		private function onScreenShotError(e:Event):void {
			gameMain.addPopup(new PopupScreenShot(gameMain, {error: "Imgur_UploadFailed"}), true);
			imgur.removeEventListener(Imgur.LOAD_COMPLETE, onScreenShotComplete);
			imgur.removeEventListener(Imgur.LOAD_ERROR, onScreenShotError);
		}
