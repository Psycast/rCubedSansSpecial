package arc.mp {
	...
	public class MultiplayerChat extends Component {
		...
		public static function textDullColour(colour:int, factor:Number):int {
			return	(int(((colour & 0xFF0000) >> 16) * factor) << 16) |
					(int(((colour & 0x00FF00) >> 8) * factor) << 8) |
					int((colour & 0x0000FF) * factor);
		}
		...
		public static function textFormatLevel(user:Object):String {
			...
			var division : int;
			var title : String;
			if (user.userLevel > 121) { color = divisionColor[7]; title = divisionTitle[7]; division = 8; }
			else if (user.userLevel >= 95) { color = divisionColor[6]; title = divisionTitle[6]; division = 7; }
			else if (user.userLevel >= 83) { color = divisionColor[5]; title = divisionTitle[5]; division = 6; }
			else if (user.userLevel >= 69) { color = divisionColor[4]; title = divisionTitle[4]; division = 5; }
			else if (user.userLevel >= 59) { color = divisionColor[3]; title = divisionTitle[3]; division = 4; }
			else if (user.userLevel >= 50) { color = divisionColor[2]; title = divisionTitle[2]; division = 3; }
			else if (user.userLevel >= 26) { color = divisionColor[1]; title = divisionTitle[1]; division = 2; }
			else { color = divisionColor[0]; title = divisionTitle[0]; division = 1; }
