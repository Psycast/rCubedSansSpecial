package arc.mp {
	...
	public class MultiplayerPanel extends MenuPanel {
		...
		public function updateWindowTitle(room:Object):void { // Minus 2 Rooms due to The Entrence (Fake) and Lobby
		...
		private function nameRoom(room:Object):String {
			...
			*/
			...
			var division : int;
			var title : String;
			if (room.level > 121) { color = divisionColor[7]; title = divisionTitle[7]; division = 8; }
			else if (room.level >= 95) { color = divisionColor[6]; title = divisionTitle[6]; division = 7; }
			else if (room.level >= 83) { color = divisionColor[5]; title = divisionTitle[5]; division = 6; }
			else if (room.level >= 69) { color = divisionColor[4]; title = divisionTitle[4]; division = 5; }
			else if (room.level >= 59) { color = divisionColor[3]; title = divisionTitle[3]; division = 4; }
			else if (room.level >= 50) { color = divisionColor[2]; title = divisionTitle[2]; division = 3; }
			else if (room.level >= 26) { color = divisionColor[1]; title = divisionTitle[1]; division = 2; }
