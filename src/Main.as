...
package {
	...
	public class Main extends MenuPanel {
		...
		private function gameInit(e:Event = null):void {
			...
			// Do Domain Locking/Local Play
			CONFIG::not_air {
				CONFIG::release {
					var url:String = (ExternalInterface.available ? ExternalInterface.call("window.location.href.toString") : loaderInfo.loaderURL);
					if (!url || url == "null") url = loaderInfo.loaderURL;
					var allowedDomains:String = "www.flashflashrevolution.com|flashfla.sh";
					var allowedPattern:String = "^http(|s)://" + allowedDomains + "/";
					var domainCheck:RegExp = new RegExp(allowedPattern, "i");
					if (!domainCheck.test(url) || Security.sandboxType.indexOf(Security.REMOTE) == -1) {
						return;
					}
				}
			}
			...
			//- Background
			var bg:GameBackgroundColor = new GameBackgroundColor();
			this.addChild(bg);
			...
			// Holidays!
			var d:Date = new Date();
			if (d.getMonth() == 0 && d.getDate() == 1)	 ver.text = "Happy New Year! - " + ver.text;
			if (d.getMonth() == 9 && d.getDate() == 31)	 ver.text = "Happy Halloween! - " + ver.text;
			if (d.getMonth() == 11 && d.getDate() == 25) ver.text = "Merry Christmas! - " + ver.text;
			if (d.getMonth() == 10 && d.getDate() == 6)	 ver.text = "Happy Birthday Velocity! - " + ver.text;
			
			//- Context Menu
			// Override Flash default right-click
			//stage.addEventListener("rightClick", toggleContextPopup);
			
			// Backup Menu incase
			var cm:ContextMenu = new ContextMenu();
			//var ssigcmi:ContextMenuItem = new ContextMenuItem("Take ScreenShot - Imgur");
			//ssigcmi.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT, takeScreenShotImgur);
			//cm.customItems.push(ssigcmi);
			
			//var ssffrcmi:ContextMenuItem = new ContextMenuItem("Take ScreenShot - FFR Photos");
			//ssffrcmi.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT, takeScreenShotFFR);
			//cm.customItems.push(ssffrcmi);
			...
			CONFIG::release { cm.hideBuiltInItems(); }
			...
			//- No Reason
			CONFIG::debug { addAlert("Development Build - " + CONFIG::timeStamp + " - NOT FOR RELEASE", 120, Alert.RED); }
		}
		...
		private function updateLoaderText():void  {
			if(loadStatus != null)
				loadStatus.htmlText = "Total: " + loadScripts + " / " + loadTotal + "\n" +
					...
					//"Friend Data: " + getLoadText(_friends.isLoaded(), _friends.isError()) + "\n" +
		...
		private function getLoadText(isLoaded:Boolean, isError:Boolean):String {
			if (isError) return "<font color=\"#FFC4C4\">Error</font>";
			if (isLoaded) return "<font color=\"#C4FFCD\">Complete</font>";
			var cycle:int = 35;
		...
		///- ScreenShot Handling
		private function takeScreenShotImgur(e:ContextMenuEvent = null):void {
			_gvars.takeScreenShot({o: true, s: 2});
		}
		
		private function takeScreenShotFFR(e:ContextMenuEvent = null):void {
			_gvars.takeScreenShot({o: true, s: 3});
		}
		...
		private function keyboardKeyDown(e:KeyboardEvent):void {
			...
			if (loadComplete && !disablePopups) {
				if (keyCode == _gvars.playerUser.keyOptions) { // Scroll Lock
