	...
	public class GamePlay extends MenuPanel {
		...
		override public function init():Boolean {
			...
			CONFIG::air {
				if(_gvars.sql_connect && !options.isEditor && !options.replay) {
					options.fill(); // Reset
					SQLQueries.getSongDetails(_gvars.sql_conn, {"engine": (song.entry.engine != null ? song.entry.engine : "ffr"), "song_id": song.entry.level}, function(results:Vector.<SQLSongDetails>):void {
		...
		override public function stageAdd():void {
			...
			if (options.isEditor) {
				...
				stage.addEventListener(Event.ENTER_FRAME, editorOnEnterFrame, false, 0, true);
				stage.addEventListener(KeyboardEvent.KEY_DOWN, editorKeyboardKeyDown, false, 0, true);
			}
			else {
				stage.frameRate = song.frameRate;
				stage.addEventListener(Event.ENTER_FRAME, onEnterFrame, false, 0, true);
				stage.addEventListener(KeyboardEvent.KEY_DOWN, keyboardKeyDown, false, 0, true);
				stage.addEventListener(KeyboardEvent.KEY_UP, keyboardKeyUp, false, 0, true);
			}
		...
		private function initCore():void {
			...
			// Sans Special Level (Modified to run on any level ID)
			//if(song.id == 2834 && song.entry.engine == null)
		...
		private function spectateSync():void {
			...
			if (gameProgress < lowIndex - 30) {
				if (highIndex - lowIndex < 60) lowIndex = lowIndex + (highIndex - lowIndex) / 2; else lowIndex += 15;
				...
				if (highIndex - lowIndex < 60) highIndex = lowIndex + (highIndex - lowIndex) / 2; else if (highIndex > 15) highIndex -= 15;
				absoluteStart = currentTime;
		...
		private function onEnterFrame(e:Event):void {
			/*
			if (keyHints) {
				for each(var hint:Alert in keyHints) {
					hint.progress();
					if (hint.alpha <= 0) {
						keyHints.splice(keyHints.indexOf(hint), 1);
						noteBox.removeChild(hint);
						hint = null;
					}
				}
			...
			switch (GAME_STATE) {
				case GAME_PLAY:
					if (legacyMode) {
						...
						if (options.replay) threshold = 0x7fffffff;
		...
		private function keyboardKeyDown(e:KeyboardEvent):void {
			...
			} else if (keyCode == 19 && (CONFIG::debug || _gvars.playerUser.isAdmin || options.replay)) { // Pause
				togglePause();
			} else if (keyCode == Keyboard.F8 && (CONFIG::debug || _gvars.playerUser.isAdmin)) {
		...
		private function endGame():void {
			...
			if (!mpSpectate && !options.isEditor) {
				...
				_gvars.songResults.push({
					"game_index": _gvars.gameIndex++,
					"level": song.id,
					"songFile": song,
					"song": song.entry,
					"amazing": hitAmazing,
		...
		private function commitJudge(dir:String, frame:int, score:int):void {
			...
			if (options.multiplayer) {
				dispatchEvent(new SFSEvent(Multiplayer.EVENT_GAME_UPDATE, {
					gameScore: gameScore,
					gameLife: gameLife,
		...
		private function multiplayerDiff(id:int, data:Object):Object {
			...
			var diff:Object = {
				amazing: data.amazing - previousDiff.amazing,
